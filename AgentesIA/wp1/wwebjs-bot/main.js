...
async function sendMessageToContact(number, message, id) {
  ...
  try {
    ...
    console.error('Error fetching group chat or participants:', error);
    axios.get('http://localhost/gestur/public/index.php/api/marketing/campana/envio/' + id)
      .then(async (response) => {
...
client.on('message_create', async message => {
  if (message.body === 'Llenar') {
    axios.get('http://localhost/gestur/public/index.php/api/marketing/campana/mensajes/8')
      .then(async (response) => {
        nroMensajes = 100;
    ...
    const intervalId = setInterval(async () => {
      ...
      for (let index = 0; index < tiempoAletorios.length; index++) {
        if (seconds === tiempoAletorios[index]) {
          axios.get('http://localhost/gestur/public/index.php/api/marketing/campana')
            .then(async (response) => {
              movil = response.data.destino.movil1;
              //movil = '573165325184';
              console.info(":::: " + movil)

              if (movil != "57no") {
                hotel = response.data.destino.razonsocialestablecimiento
                ...
                mensaje = + "Cada nombre que me mencionan venía acompañado de una historia de lucha, resiliencia, creatividad y amor profundo por Nuquí 💪🏽🌴💙";
                ...
                await sleep(10000);
                axios.get('http://localhost/gestur/public/index.php/api/marketing/campana/envio/' + response.data.id)
                  .then(async (response) => {
